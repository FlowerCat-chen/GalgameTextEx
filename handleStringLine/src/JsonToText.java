...
public class JsonToText {
	...
	static String[] tmpText;
		public static void main(String[] args) {
			...
			String inputFolder = "/storage/emulated/0/红莲华script/"; // 输入文件夹路径
			String outputFolder = "/storage/emulated/0/红莲华script/txtout/"; // 输出文件夹路径
			String jsonFolder = "/storage/emulated/0/红莲华script/gt_output/";
...
if(!line.isEmpty()){
	         ...
	         index++;
	         tmpText=JsonReader.getjsonContent(index);
					
					if(line.contains("<")&&line.contains(">")&&!line.contains("<r>")){
                      
						writer.write(line.substring(0,line.indexOf(">")+1)+tmpText[1].replace("<|im_end|>","")); // 写入输出文件
						writer.newLine();
						//Thread.sleep(10);
					} else {
		...
		if(line.contains("<r>")){}
						...
						// 其它情况下原封不动地输出到输出文件
						writer.write(tmpText[1].replace("<|im_end|>",""));
						writer.newLine();
					}
				}
}
